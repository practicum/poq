...
% int_type and and guidtype are not.  Ideally, we would have
...
nonnull(X) :- \+int_type(X), \+name_type(X), \+wordstr(X), \+guidtype(X), \+natural_type(X), \+tinyint(X),  \+barcodeEnum(X),
              \+isnull(X).
...
% without the following, other atoms not 'typed' in this file will FAIL a 'nonnull' test.
% for example:  nonnull(abcdefghijk).  will come out FALSE.
getnonnull(X) :- \+int_type(X), \+name_type(X), \+wordstr(X), \+guidtype(X), \+natural_type(X), \+tinyint(X),  \+barcodeEnum(X),
              \+isnull(X).
